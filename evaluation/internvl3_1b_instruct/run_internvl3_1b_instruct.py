...
def run_internvl3_1b_instruct(sample, model, tokenizer):
    device = model.device
    dtype = model.dtype
    image = sample['image']
...
if __name__ == "__main__":
    ...
    model = AutoModel.from_pretrained(
        ...
        trust_remote_code=True).eval().to("cuda:0")
    ...
    for dataset_name, dataset in datasets.items():
        for sample in tqdm(dataset, desc=f"Processing {dataset_name}"):
            output_text, answer = run_internvl3_1b_instruct(sample, model, tokenizer)
            question = sample['question']
            answer = sample['answer']
